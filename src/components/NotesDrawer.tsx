...
export function NotesDrawer() {
  ...
  return (
    <>
      <div
        className={`fixed inset-0 z-40 bg-navy/45 backdrop-blur-sm transition-opacity ${
          state.notesOpen ? 'opacity-100' : 'pointer-events-none opacity-0'
        }`}
        onClick={() => dispatch({ type: 'toggleNotes', open: false })}
      />
      <aside
        className={`fixed top-0 right-0 z-50 flex h-full w-full max-w-sm flex-col border-l border-line bg-white shadow-2xl transition-transform ${
          state.notesOpen ? 'translate-x-0' : 'translate-x-full'
        }`}
      >
        <div className="flex items-center justify-between bg-navy p-4 text-white">
          <h2 className="text-sm font-bold uppercase">Notes History & Audit</h2>
          <button
            type="button"
            onClick={() => dispatch({ type: 'toggleNotes', open: false })}
        ...
        <div className="custom-scroll flex-1 space-y-3 overflow-y-auto bg-mist p-4">
          {notes.length === 0 && (
          ...
          {notes.map((note) => {
            ...
            return (
              <article key={note.id} className="lp-panel p-4">
                <div className="mb-2 flex items-start justify-between gap-2">
                  <p className="text-xs font-bold text-ink">{note.subject}</p>
